...
public class RNSshClientModule extends ReactContextBaseJavaModule {
  ...
  @ReactMethod
  private void connectToHostByPassword(
    final String host,
    final Integer port,
  ...
  @ReactMethod
  private void connectToHostByKey(
    final String host,
    final Integer port,
    final String username,
    final ReadableMap passwordOrKey,
    final String key,
    final Callback callback
  ) {
        ...
        public void run() {
          try {
            ...
            if (password == null) {
              ...
              byte[] publicKey = keyPairs.hasKey("publicKey")
                ? keyPairs.getString("publicKey").getBytes()
                : null;
              byte[] passphrase = keyPairs.hasKey("passphrase")
                ? keyPairs.getString("passphrase").getBytes()
                : null;
      ...
      new Runnable() {
        ...
        public void run() {
          try {
            ...
            client._bufferedReader =
              new BufferedReader(new InputStreamReader(in));
            client._dataOutputStream =
              new DataOutputStream(channel.getOutputStream());
            ...
            while (
              ...
              (line = client._bufferedReader.readLine()) != null
            ) {
    ...
    new Thread(
      new Runnable() {
        ...
        public void run() {
          try {
            SSHClient client = clientPool.get(key);
            ChannelSftp channelSftp = (ChannelSftp) client._session.openChannel(
              "sftp"
            );
            channelSftp.connect();
        ...
        public void run() {
          try {
            ...
            for (LsEntry file : files) {
              ...
              if (
                filename.trim().equals(".") || filename.trim().equals("..")
              ) continue;
        ...
        public void run() {
          try {
            ...
            channelSftp.get(
              filePath,
              path,
              new progressMonitor(key, "DownloadProgress")
            );
        ...
        public void run() {
          try {
            ...
            channelSftp.put(
              ...
              new progressMonitor(key, "UploadProgress"),
